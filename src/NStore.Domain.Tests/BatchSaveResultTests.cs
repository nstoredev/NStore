...
namespace NStore.Domain.Tests
{
    public class BatchSaveResultTests : BaseBatchRepositoryTest
    {
        ...
        public async Task SaveManyAsync_should_return_populated_result_for_single_changed_aggregate()
        {
            ...
            // Act
            var result = await BatchRepository.SaveManyAsync(tickets.Values.ToList(), "tdd_op").ConfigureAwait(false);
        ...
        public async Task SaveManyAsync_should_return_multiple_results_for_multiple_aggregates()
        {
            ...
            // Act
            var result = await BatchRepository.SaveManyAsync(tickets.Values.ToList(), "batch_op").ConfigureAwait(false);
        ...
        public async Task SaveManyAsync_should_return_unchanged_result_when_no_changes()
        {
            ...
            // Act
            var result = await BatchRepository.SaveManyAsync(tickets.Values.ToList(), "no_change_op").ConfigureAwait(false);
            ...
            Assert.Equal(AggregateSaveOutcome.Unchanged, res.Outcome);
        }
        ...
        public async Task SaveManyAsync_should_report_concurrency_failure_in_result()
        {
            ...
            var tickets = await BatchRepository.GetManyByIdAsync<Ticket>(new[] { "Ticket_1" }).ConfigureAwait(false);
            tickets["Ticket_1"].Sale();
            await BatchRepository.SaveManyAsync(tickets.Values.ToList(), "first").ConfigureAwait(false);
            ...
            await BatchRepository.SaveManyAsync(tickets1.Values.ToList(), "second").ConfigureAwait(false);

            tickets2["Ticket_1"].DoSomething();

            // Act - second repo saves with stale version
            var result = await repo2.SaveManyAsync(tickets2.Values.ToList(), "third").ConfigureAwait(false);
            ...
            Assert.Equal(AggregateSaveOutcome.Concurrency, res.Outcome);
        }
        ...
        public async Task SaveManyAsync_should_report_partial_success_with_mixed_results()
        {
            ...
            await BatchRepository.SaveManyAsync(tickets.Values.ToList(), "init").ConfigureAwait(false);
            ...
            await repo1.SaveManyAsync(tickets1.Values.ToList(), "conflict_setup").ConfigureAwait(false);
            ...
            // Act
            var result = await repo2.SaveManyAsync(tickets2.Values.ToList(), "partial").ConfigureAwait(false);
            ...
            Assert.All(failed, r => Assert.Equal(AggregateSaveOutcome.Concurrency, r.Outcome));

            Assert.All(succeeded, r => Assert.NotNull(r.Chunk));
            Assert.All(succeeded, r => Assert.Null(r.Outcome));
        }
    }
}
